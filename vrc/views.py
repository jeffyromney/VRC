...
def addVolunteer(request):
    ...
    if request.method == 'POST':   #If form is being submitted
    	if ('continue' in request.GET) and request.GET.get('continue') == '1':
    		form = VolunteerForm(request.session.get('formBackup'))
    		try:
    			del request.session['formBackup']
    		except KeyError:
     			pass
    		print(form)
    		print("Continue....")
        ...
        if form.is_valid():
            ...
            if dups:
            	if ('continue' in request.GET) and request.GET.get('continue') == '1':
            		...
            		request.session['formBackup'] = request.POST
                	return render(request, 'duplicateCheck.html', {'form':form, 'duplicates':dups})
            else:
                new_Volunteer = form.save() #save to database
                LogEntry.objects.log_action(
                            user_id = request.user.username,
